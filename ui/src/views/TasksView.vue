<template>
  <div class="card">
    <div class="card-header">
      ...
      <button class="btn btn-primary" @click="taskVisible = true">➕ 提交任务</button>
    </div>
    <el-table :data="tasks" size="small" v-loading="loading">
      ...
      <el-table-column label="操作" width="120">
        <template #default="{ row }">
          <button class="btn btn-primary btn-sm" v-if="row.status === 'running'">停止</button>
    ...
    <el-dialog v-model="taskVisible" title="➕ 提交任务" width="500px">
      ...
      <template #footer><button class="btn btn-secondary" @click="taskVisible = false">取消</button><button class="btn btn-primary" @click="submitTask">提交</button></template>
    </el-dialog>
  </div>
</template>
