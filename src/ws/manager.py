...
class WebSocketManager:
    ...
    async def connect(self) -> asyncio.Queue:
        ...
        logger.debug(
            f"WebSocket 连接建立，当前连接数: {len(self._active_connections)}"
        )
        return queue
    ...
    async def disconnect(self, queue: asyncio.Queue) -> None:
        ...
        logger.debug(
            f"WebSocket 连接断开，当前连接数: {len(self._active_connections)}"
        )
    ...
    async def send_task_update(
        self, task_id: str, status: str, progress: float, **kwargs
    ) -> None:
        """发送任务更新"""
        message = {
            "type": "task_update",
    ...
    def get_subscription_count(self, task_id: Optional[str] = None) -> int:
        ...
        total = sum(
            len(subscribers)
            for subscribers in self._task_subscriptions.values()
        )
        return total
